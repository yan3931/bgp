...
### 5.1 子游戏专属榜单：贝叶斯平滑与环境引力
单游戏排行榜核心解决一个痛点：**“1 场 1 胜的 100% 胜率萌新”不应该排在“100 场 60 胜的 60% 胜率大佬”上面。**

1. **贝叶斯平滑胜率 ($\hat{p}_g$)**
   引入先验数据，将玩家的原始胜率向该款游戏的“客观难度基准”拉扯：

   $$ \hat{p}_g = \frac{w_g + \lambda \times b_g}{n_g + \lambda} $$

   - $w_g$：该位玩家在此游戏的胜场。
   - $n_g$：该位玩家在此游戏的局数。
   - $b_g$：游戏的环境基准胜率 (Baseline，通常是 $\frac{1}{\text{推荐人数}}$，合作类则是整体通关率)。
   - $\lambda$：收缩系数 (当前设定为 2.0)，相当于塞给玩家两局表现平平的虚拟记录。
   *意义：萌新打赢 1 场，平滑后胜率会从 100% 暴跌回 50% 左右；随着百战百胜，平滑效应趋近于 0，胜率回归真实水平。*

2. **Logit 空间防刷分插值与可信度锁 ($w_N$)**
   仅有平滑还不够，为鼓励深耕一款游戏并提供可展示的“保守战力分”，我们借由天梯分式求得 $P_{ladder}$：

   $$ P_{ladder} = \sigma\big((1-w_N)\text{logit}(b_g) + w_N\text{logit}(\hat{p}_g)\big) $$
   $$ w_N = \frac{n_g}{n_g + k} $$

   - $w_N$：局数依赖的“可靠性锁”（或滑动变阻器）。随着局数 $n_g$ 增多（$k=5$），$w_N$ 从 0 滑向 1。
   - $\text{logit}(x) = \ln(\frac{x}{1-x})$：将 [0,1] 的胜率映射到实数域 $(-\infty, \infty)$。
   - $\sigma(x) = \frac{1}{1+e^{-x}}$：Logit 的反函数，把实数域拉回到 [0,1] 提供展示用的百分比。
   *意义：玩家初次游玩时，系统完全不信任个人的超常发挥（$w_N \approx 0$），天梯分死死锁定在游戏客观难度 $b_g$；只有当你的局数积累到一定规模（$n_g \ge 5$ 时解锁“专精”徽章），你的真实展现 $\hat{p}_g$ 才能完全冲破这层枷锁，成为排行榜排序的真正依据。*

### 5.2 全局总战力：跨游双轨与动态权重
在 Global 层面，玩家涉猎的游戏不仅有难度（权重）之分，更有 **PvP（互殴对抗）** 与 **PvE（合作解谜）** 的类型之别。

1. **游戏权重测算 ($w_{static}$)**
   每款游戏在 `GAME_REGISTRY` 中有一个基础权重 $W$，但简单把权重乘以胜率会导致“高难长线游戏”的战力无限膨胀。
   因此我们引入 Softplus 平滑压缩：

   $$ w_{static}^{(g)} = \text{softplus}\big(\alpha (W_g - 1)\big) + \beta $$

   *这使得“15 分钟的狼人杀”和“3 小时的重策”在战力池中会有阶梯式的差距，但不会断层拉开。*

2. **双轨分离计算**
   - **PvP 赛道优势护城河**：由于对抗类游戏零和博弈本质，引入了净优势：$\Delta_g = \text{logit}(\hat{p}_g) - \text{logit}(b_g)$，最后再 Sigmoid 挤压回去。只奖励“超越平均环境表现”的人，如果你一直输，你的 PvP 战力会无限趋近于 0，拉低你的平均池。
   - **PvE 赛道无损通关率**：合作类不具备零和剥夺属性（没打过就是没打过），因此摒弃 Logit 优势映射，严格使用 `(平滑胜率 * 严格出勤防云锁)` 的双重压制。

3. **加权大锅饭与战力展示**
   玩家的最终战力其实就是其在两大赛道所有参与游戏的加权平均分之和：

   $$ \text{Total Combat Power} = \sum (\text{PvP Score} \times w_{static}) + \sum (\text{PvE Score} \times w_{static}) $$

   最终，展示在最外围的就是这经受了重重脱水后、真正反映一名玩家综合多边形硬实力的“干货数据”。
